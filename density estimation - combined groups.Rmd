...
We load the data that was already preprocessed in the scripts "minimal_preprocessing" and "preprocessing_continued". For the illustration of the methods we will work with the assay "peptidesSweepImp" from the original data in "leduc_sces".
...
We define the data we need to fit the model: it contains the discretised sequence, the counts and the carrier. We will use a non-normalised uniform carrier. Then we can fit the desired model. We include the intensities up to the fourth degree for the GLM Poisson model and we also fit a smoothing GAM model.
...
We can then plot the histogram of the counts and the predicted counts in each interval under the two models.
...
We now plot frequency histograms where we plot the predicted densities for a sequence of 2000 new data points.
...
We want to create a plot that compares the densities predicted by the special exponential family method and a kernel density method.
...
We create a plot where we fit models of different flexibility on the intensities of one of the peptides:
